...
import libreria.libreria.orders.util.ItemQuantity;
import libreria.libreria.orders.util.OrderCancel;
...
public class OrderController {
    ...
    public ResponseEntity<?> order(
        ...
        if (ItemQuantity.isSoldOut(item.getRemaining())) {
            log.info("품절입니다.");
            return ResponseEntity.ok("품절된 상품입니다. 상품 홈으로 돌아가주세요");
        }

        if (ItemQuantity.isOverRemaining(
                item.getRemaining(),
